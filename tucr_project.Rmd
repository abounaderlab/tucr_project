...
<<<<<<< HEAD
p <- ggplot(plot_annotation, aes(x=annot,y=count,fill=annot)) 
  + geom_bar(stat="identity", width = 0.7) 
  + geom_text(aes(label=count), vjust=1.6, color="black", size=7) 
  + scale_fill_manual(values=c("#F8766D","#00BFC4","#7CAE00","#C77CFF")) 
  + ggtitle(paste("TUCR Genomic Location")) 
  + xlab("Genomic Location relative to nearest gene") 
...
>>>>>>> efea12d00cff4703a47aef8c80036b5c87520470
...
<<<<<<< HEAD
ggsave(file=paste(outputdir,"/tucr_annotation.png",sep=""),plot = print(p)
...
p <- ggplot(tucr_lncRNAs, aes(x=row,y=value,fill=row)) 
  + geom_bar(stat="identity", width = 0.7) 
  + geom_text(aes(label=round(value,2)), vjust=1.6, color="black", size=rel(4)) 
...
ggsave(file=paste(outputdir,"/tucr_lncRNAs.png",sep=""),plot = print(p),
       width = 5, height = 5, dpi = 300) 
...
ggsave(file=paste(outputdir,"/",disease,"_deregGraph.png",sep=""),plot = print(p),
       width = 5, height = 5, dpi = 300)
...
```{r DESeq2plots_TUCRs, message = FALSE, eval = FALSE}
...
```{r DESeq2plots_intergenicTUCRs, eval = FALSE}
...
```{r surcor_TUCRs, message=FALSE, eval = FALSE}
...
```{r volcanosurv TUCRs, eval = FALSE}
...
```{r RPKM_TUCRs, results="hide", eval = FALSE}
...
```{r boxplot_TUCRs, results = "hide", eval = FALSE}
