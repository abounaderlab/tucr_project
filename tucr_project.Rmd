...
n_metadatafile <- paste(normal,"tcga_metadata.csv",sep="_")
...
if(!dir.exists("CHESSBED")){dir.create("CHESSBED")}
...
for(i in 1:length(chrom)){
  x <- bed %>%
    filter(V1 == chrom[i]) %>%
    write.table(paste("CHESSBED/",chrom[i],"_CHESSgenes.bed",sep=""),quote = F,col.names = F,row.names = F,sep = '\t')
}}

bedbreaker("CHESSGenes.bed")
}
...
#Write header line
echo -e chrom"\t"start"\t"end"\t"id RNASEQ-PROCESSED/gtex_analysis/cortex/reindex/*bam | sed -e "s/ /\t/g" 
  > RNASEQ-PROCESSED/gtex_analysis/cortex/foldchange/$name.counts.txt

#Counts
multiBamCov -bams RNASEQ-PROCESSED/gtex_analysis/cortex/reindex/*bam -bed $bed -q 10
  >> RNASEQ-PROCESSED/gtex_analysis/cortex/foldchange/$name.counts.txt

done

head -1 RNASEQ-PROCESSED/gtex_analysis/cortex/foldchange/chr10_CHESSgenes.counts.txt > RNASEQ-PROCESSED/gtex_analysis/cortex/foldchange/cortex_mergedcounts.txt; tail -n +2 -q RNASEQ-PROCESSED/gtex_analysis/cortex/foldchange/*.counts.txt >> RNASEQ-PROCESSED/gtex_analysis/cortex/foldchange/cortex_mergedcounts.txt
...
#Write header line
echo -e chrom"\t"start"\t"end"\t"id RNASEQ-PROCESSED/tcga_analysis/GBM/reindex/*bam | sed -e "s/ /\t/g" 
  > RNASEQ-PROCESSED/tcga_analysis/GBM/foldchange/$name.counts.txt

#Counts
multiBamCov -bams RNASEQ-PROCESSED/tcga_analysis/GBM/reindex/*bam -bed $bed -q 10
  >> RNASEQ-PROCESSED/tcga_analysis/GBM/foldchange/$name.counts.txt

done

head -1 RNASEQ-PROCESSED/tcga_analysis/GBM/foldchange/chr10_CHESSgenes.counts.txt > RNASEQ-PROCESSED/tcga_analysis/GBM/foldchange/GBM_mergedcounts.txt; tail -n +2 -q RNASEQ-PROCESSED/tcga_analysis/GBM/foldchange/*.counts.txt >> RNASEQ-PROCESSED/tcga_analysis/GBM/foldchange/GBM_mergedcounts.txt
...
#Write header line
echo -e chrom"\t"start"\t"end"\t"id RNASEQ-PROCESSED/tcga_analysis/LGG/reindex/*bam | sed -e "s/ /\t/g" > RNASEQ-PROCESSED/tcga_analysis/LGG/foldchange/$name.counts.txt

#Counts
multiBamCov -bams RNASEQ-PROCESSED/tcga_analysis/LGG/reindex/*bam -bed $bed -q 10 >> RNASEQ-PROCESSED/tcga_analysis/LGG/foldchange/$name.counts.txt

done

head -1 RNASEQ-PROCESSED/tcga_analysis/LGG/foldchange/chr10_CHESSgenes.counts.txt > RNASEQ-PROCESSED/tcga_analysis/LGG/foldchange/LGG_mergedcounts.txt; tail -n +2 -q RNASEQ-PROCESSED/tcga_analysis/LGG/foldchange/*.counts.txt >> RNASEQ-PROCESSED/tcga_analysis/LGG/foldchange/LGG_mergedcounts.txt
...
if(!is.na(filterannot)){
  mergedcounts <- mergedcounts %>%
    filter(annot == filterannot)
}


mergedcounts2 <- 
  mergedcounts[,7:ncol(mergedcounts)]
...
mergedcounts.info$length <- 
  (mergedcounts$end - mergedcounts$start)/1000
...
write.csv(res, file = paste(outputdir,"/ExpressionAnalysis/",disease,"_results_",filterannot,".csv",sep=""))}else{
write.csv(res, file = paste(outputdir,"/ExpressionAnalysis/",disease,"_results_allgenes.csv",sep=""))
}
...
volcanoplot(res_TUCR,genes=c("uc.110","uc.210","uc.427","uc.62"),
            title = paste("Deregulated Genes in ",disease,sep=""), 
            output = paste(outputdir,"/ExpressionAnalysis/",disease,"_myron_tucr_results_volcanoplot.png",sep=""))
...
volcanoplot(res_TUCR,
            title = paste("Deregulated Genes in ",disease,sep=""), 
            output = paste(outputdir,"/ExpressionAnalysis/",disease,"_tucr_results_volcanoplot.png",sep=""))
...
write.csv(res_intergenic, file = paste(outputdir,"/ExpressionAnalysis/",disease,"_results_intergenicOnly.csv",sep=""))

volcanoplot(res_intergenic,genes=c("uc.62","uc.110"), 
            title = paste("Intergenic TUCR deregulation in ",disease,sep=""),
            output = paste(outputdir,"/ExpressionAnalysis/",disease,"_intergenic_tucr_results_volcanoplot.png",sep=""))

res_exonic <- inner_join(res_TUCR,tucr_annot,by = "row") %>%
  filter(annot == "exonic")

write.csv(res_exonic, file = paste(outputdir,"/",disease,"_results_exonicOnly.csv",sep=""))

volcanoplot(res_exonic,genes=c("uc.90","uc.213"), 
            title = paste("exonic TUCR deregulation in ",disease,sep=""),
            output = paste(outputdir,"/ExpressionAnalysis/",disease,"_exonic_tucr_results_volcanoplot.png",sep=""))

res_intronic <- inner_join(res_TUCR,tucr_annot,by = "row") %>%
  filter(annot == "intronic")

write.csv(res_intronic, file = paste(outputdir,"/ExpressionAnalysis/",disease,"_results_intronicOnly.csv",sep=""))

volcanoplot(res_intronic,genes=c("uc.430","uc.275"), 
            title = paste("intronic TUCR deregulation in ",disease,sep=""),
            output = paste(outputdir,"/ExpressionAnalysis/",disease,"_intronic_tucr_results_volcanoplot.png",sep=""))

res_exonic_intronic <- inner_join(res_TUCR,tucr_annot,by = "row") %>%
  filter(annot == "exonic_intronic")

write.csv(res_exonic_intronic, file = paste(outputdir,"/ExpressionAnalysis/",disease,"_results_exonic_intronicOnly.csv",sep=""))

volcanoplot(res_exonic_intronic,genes=c("uc.282","uc.341"), 
            title = paste("exonic/intronic TUCR deregulation in ",disease,sep=""),
            output = paste(outputdir,"/ExpressionAnalysis/",disease,"_exonic_intronic_tucr_results_volcanoplot.png",sep=""))
...
normal_mergedcounts <- read.table("normal_mergedcounts.txt",header = TRUE) %>%
  dplyr::select(-chrom,-start,-end)

mergedcounts <- left_join(mergedcounts,normal_mergedcounts,by="id")

metadata <- 
  read_csv(file = metadatafile) 

#CHESSantisense <- read.table("BEDFiles/CHESSgenes.bed") %>%
#  filter(V6 == "antisense_RNA") %>%
#  dplyr::select("V5","V6")

#CHESScoding <- read.table("BEDFiles/CHESSgenes.bed") %>%
#  filter(V6 == "protein_coding") %>%
#  dplyr::select("V5","V6")

#CHESSlncRNA <- read.table("BEDFiles/CHESSgenes.bed") %>%
#  filter(V6 == "lncRNA") %>%
#  dplyr::select("V5","V6")


#CHESSmisc <- read.table("BEDFiles/CHESSgenes.bed") %>%
#  filter(V6 == "misc_RNA") %>%
#  dplyr::select("V5","V6")

#TUCRids <- tucr_annot <- read.table("BEDFiles/hg38.ultraconserved.bed") %>%
#  dplyr::select("row" = V5, "annot" = V7)
...
write.csv(rpkm.df2,paste(outputdir,"/ExpressionAnalysis/",disease,"_rpkm_",annotation,".csv",sep=""))

write.csv(rpkm.median,paste(outputdir,"/ExpressionAnalysis/",disease,"_medianrpkm_",annotation,".csv",sep=""))
...
ggsave(p,file=paste(outputdir,"/ExpressionAnalysis/",disease,"_",annotation,"_heatmap.png",sep=""), width = 2.5, height = 7, dpi = 300)
...
if(!dir.exists(paste(outputdir,"/ExpressionAnalysis/RPKMboxplots",sep=""))){
  dir.create(paste(outputdir,"/ExpressionAnalysis/RPKMboxplots",sep=""))
}
...
rpkm.dotplot <- rpkm.TUCRs2[,8:ncol(rpkm.TUCRs2)]
...
rpkm.dotplot <- rpkm.dotplot %>% 
  ...
  mutate(Key = gsub("[.]","-",Sample)) %>%
    mutate(string = sub(".*TCGA", "", Key),barcode = tolower(paste("TCGA",substr(string, 1, 8),sep=""))) %>%
  left_join(metadata,by="barcode") 

rpkm.dotplot.nmeans <- rpkm.dotplot %>%
  filter(dex == "normal") %>%
  group_by(TUCRids) %>%
  mutate(mean = mean(RPKM,na.rm=TRUE)) %>%
  dplyr::select(TUCRids,mean) %>%
  distinct()

res_TUCR <- read_csv(paste(outputdir,"/ExpressionAnalysis/",disease,"_results_TUCR.csv",sep="")) %>%
  dplyr::select("TUCRids"=row,log2FoldChange)

rpkm.dotplot <- rpkm.dotplot %>%
  left_join(rpkm.dotplot.nmeans,by="TUCRids") %>%
  left_join(res_TUCR,by="TUCRids") %>%
  mutate(DESeq2 = ifelse(dex=="normal",0,log2FoldChange)) %>%
  mutate(foldchange = ifelse(mean==0,RPKM/0.001,ifelse((RPKM/mean)>=0,RPKM/mean,ifelse((RPKM/mean)<0,(-1/(RPKM/mean)),NA)))) %>%
mutate(log2FC = ifelse(foldchange==0,0,log2(foldchange))) %>%
  dplyr::select(-Sample,-Key,-id,-string,-log2FoldChange) %>%
  filter(!is.na(dex))
...
p<- ggplot(rpkm.dotplot2, aes(x=dex, y=RPKM)) +
  ...
  panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
  panel.background = element_blank(), axis.line = element_line(colour = "black")) +
  labs(y="Reads per kilobase million (RPKM)", x = "Sample") +
  stat_summary(fun.y=mean, geom="point", shape=20, size=10, color="red", fill="red")

rpkm.dotplot3 <- rpkm.dotplot2 %>% 
  dplyr::select(TUCRids,DESeq2,dex) %>%
  mutate(fill = ifelse(DESeq2>=1,"red",ifelse(DESeq2<=-1,"green","snow3"))) %>%
  distinct()

rpkm.dotplot3$fill <- as.character(rpkm.dotplot3$fill)
foldchange.DESeq2 <- round(rpkm.dotplot3$DESeq2[1],2)

p2 <- ggplot(data = rpkm.dotplot3,aes(x=dex, y=DESeq2,fill=fill)) +
  geom_bar(colour="black",stat="identity") +
  scale_fill_identity() +
  #scale_color_manual(values = c("black"="black","red"="red","green"="green")) +
  geom_jitter(data = rpkm.dotplot2, aes(x=dex, y=log2FC),binaxis='y', stackdir='center', stackratio=0.90, dotsize=0.30,fill="gray") +
  ggtitle(paste("Deregulation of ",TUCR," in ",disease," (",foldchange.DESeq2,"-log2fold)",sep="")) +
  theme(plot.title = element_text(hjust = 0.5),
  panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
  panel.background = element_blank(), axis.line = element_line(colour = "black")) +
...
ggsave(p,file=paste(outputdir,"/ExpressionAnalysis/RPKMboxplots/",disease,"_",TUCR,"_rpkm_boxplot.png",sep = ""))
ggsave(p2,file=paste(outputdir,"/ExpressionAnalysis/FCplots/",disease,"_",TUCR,"_FC_plot.png",sep = ""))
}
...
write_csv(surv_TUCR,paste(outputdir,"/SurvivalAnalysis/",disease,"_survival_TUCRs.csv",sep=""))
...
for(i in 1:length(TUCRids)){
  ...
  if(skip_to_next) { 
    ...
    ptable[i,] <- rbinder
    next } 
  ggsave(file=paste(outputdir,"/SurvivalAnalysis/tucrKPMplots/",disease,"_",TUCR,"_kpmplot.png",sep = ""),
         plot = print(p))
...
write_csv(ptable,paste(outputdir,"/SurvivalAnalysis/",disease,"_kpm_summary_TUCRs.csv",sep=""))
...
volcanosurv <- function (res,genes = "all",title = paste("TUCRs correlated with survival in ",disease,sep=""), output = paste(disease,"_results__survival_volcanoplot.png",sep=""),height = 5, width = 7.5, dpi = 300){
  #res <- res_surv
  #genes = c("uc.110","uc.62")
...
surv_TUCR <- read.csv(paste(outputdir,"/SurvivalAnalysis/",disease,"_survival_TUCRs.csv",sep=""),header=TRUE)

ptable <- read.csv(paste(outputdir,"/SurvivalAnalysis/",disease,"_kpm_summary_TUCRs.csv",sep=""),header=TRUE)
...
for (i in 1:length(res_surv_sum2$id)){
  ...
  ifelse(res_surv_sum2$Survival[i] <- "Not significant"))))))}

res_surv_sum2 <- res_surv_sum2 %>%
  group_by(Survival) %>%
  summarise(count = n())

write.csv(res_surv_sum2,file=paste(outputdir,"/SurvivalAnalysis/",disease,"_",annotation,"_tucr_surv_table.csv",sep=""))

p <- ggplot(res_surv_sum2, aes(x=Survival,y=count,fill=Survival)) + geom_bar(stat="identity", width = 0.7) + scale_fill_manual(values = c("gray","#619cff","#f8766d","#00ba38","#c77cff","black")) + ylab("# of TUCRs") + theme(
plot.title = element_text(size=rel(1.5), face="bold",hjust = 0.5),
axis.title = element_text(size = rel(1.25), face="bold"),
panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black"),
...
ggsave(file=paste(outputdir,"/SurvivalAnalysis/",disease,"_",annotation,"_tucr_surv_bar.png",sep=""),plot = print(p), width = 5, height = 5, dpi = 300)

res_surv_sum3 <- res_surv_sum

volcanosurv(res_surv_sum3, genes = "", title = paste(annotation," TUCR correlation with patient survival in ",disease,sep=""),output = paste(outputdir,"/SurvivalAnalysis/",disease,"_",annotation,"_tucr_results_volcanosurv.png",sep=""))
...
write.csv(res_surv_sum2,file=paste(outputdir,"/SurvivalAnalysis/",disease,"_tucr_surv_table_",annotation,".csv",sep=""))
...
ggsave(file=paste(outputdir,"/SurvivalAnalysis/",disease,"tucr_surv_bar_",annotation,".png",sep=""),plot = print(p), width = 5, height = 5, dpi = 300)
