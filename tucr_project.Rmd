...
```{r cancerGO, eval = TRUE}

cancermine <- read.csv("annotated_genelist_onctsg.csv",header=TRUE)

IDs <- as.character(cancermine$target)
  
entrezIDs <- mapIds(org.Hs.eg.db, IDs, 'ENTREZID', 'SYMBOL')
  
cancermine$entrez <- entrezIDs

Genes<-entrezIDs
g <- goana(Genes)
topGO_cancermine <- topGO(g) 

cancermine_TSG <- cancermine %>%
  filter(role == "TSG")

IDs <- as.character(cancermine_TSG$target)
  
entrezIDs <- mapIds(org.Hs.eg.db, IDs, 'ENTREZID', 'SYMBOL')
  
cancermine_TSG$entrez <- entrezIDs

Genes<-entrezIDs
g <- goana(Genes)
topGO_cancermine_TSG <- topGO(g) 

cancermine_ONC <- cancermine %>%
  filter(role == "ONC")

IDs <- as.character(cancermine_ONC$target)
  
entrezIDs <- mapIds(org.Hs.eg.db, IDs, 'ENTREZID', 'SYMBOL')
  
cancermine_ONC$entrez <- entrezIDs

Genes<-entrezIDs
g <- goana(Genes)
topGO_cancermine_ONC <- topGO(g) 

topGO_cancermine_ALL <- rbind(topGO_cancermine,topGO_cancermine_ONC,topGO_cancermine_TSG)

cancer_GOTerms <- unique(as.character(topGO_cancermine_ALL$Term))
...
save(allgo,allgo_bp,allgo_mf,allgo_cc,file="allgo.rData")
```

## Get "Cancer" GO Terms only

```{r cancerGO_TUCR, eval = TRUE}

load("allgo.rData")

if(!dir.exists(paste(outputdir,"/GO_terms_bp_cancer",sep=""))){
  dir.create(paste(outputdir,"/GO_terms_bp_cancer",sep=""))
}

allgo_cancer <- allgo[which(allgo$Term %in% cancer_GOTerms),]

allgo_cancer_summary <- allgo_cancer %>%
  filter(P.DE <= 0.05) %>%
  group_by(Term,direction) %>%
  summarise(n = n())

write_csv(allgo_cancer_summary,paste(outputdir,"/GOterm_cancer_all_summary_TUCRs.csv",sep=""))

if(!dir.exists(paste(outputdir,"/GO_terms_bp_cancer",sep=""))){
  dir.create(paste(outputdir,"/GO_terms_bp_cancer",sep=""))
}

allgo_bp_cancer <- allgo_bp[which(allgo_bp$Term %in% cancer_GOTerms),]

allgo_bp_cancer_summary <- allgo_bp_cancer %>%
  filter(P.DE <= 0.05) %>%
  group_by(Term,direction) %>%
  summarise(n = n())

write_csv(allgo_bp_cancer_summary,paste(outputdir,"/GOterm_cancer_bp_summary_TUCRs.csv",sep=""))

if(!dir.exists(paste(outputdir,"/GO_terms_mf_cancer",sep=""))){
  dir.create(paste(outputdir,"/GO_terms_mf_cancer",sep=""))
}

allgo_mf_cancer <- allgo_mf[which(allgo_mf$Term %in% cancer_GOTerms),]

allgo_mf_cancer_summary <- allgo_mf_cancer %>%
  filter(P.DE <= 0.05) %>%
  group_by(Term,direction) %>%
  summarise(n = n())

write_csv(allgo_mf_cancer_summary,paste(outputdir,"/GOterm_cancer_mf_summary_TUCRs.csv",sep=""))

if(!dir.exists(paste(outputdir,"/GO_terms_cc_cancer",sep=""))){
  dir.create(paste(outputdir,"/GO_terms_cc_cancer",sep=""))
}

allgo_cc_cancer <- allgo_cc[which(allgo_cc$Term %in% cancer_GOTerms),]

allgo_cc_cancer_summary <- allgo_cc_cancer %>%
  filter(P.DE <= 0.05) %>%
  group_by(Term,direction) %>%
  summarise(n = n())

write_csv(allgo_cc_cancer_summary,paste(outputdir,"/GOterm_cancer_cc_summary_TUCRs.csv",sep=""))
...
allgo_cancer_summary <- read.csv(paste(outputdir,"/GOterm_cancer_all_summary_TUCRs.csv",sep=""),header=TRUE)
allgo_cancer_summary <- allgo_cancer_summary %>%
  dplyr::select(Term)

rbinder.allgo <- allgo_cancer_summary
...
ggsave(p,file=paste(outputdir,"/GOTerm_cancer_all_heatmap.png",sep=""))
