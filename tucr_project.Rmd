...
countdata <- read.table("tucr_counts.txt",header = TRUE)
metadata <- read_csv(file = "tcga_metadata.csv")
...
ggsave(p,file=paste(filename,".png",sep=""), width = 20, height = 5, dpi = 300)
x %>%
  select(-ID) %>%
  write_csv(paste(filename," (Output).csv",sep=""))
}
...
#tucr.cor.matrix.shave <- 
  
tucr.cor.matrix2 %>%
  corrr::correlate() %>%
  corrr::focus(1:481, mirror = TRUE) %>%
  # converts the upper triangle (default) to missing values
  corrr::shave() %>%
  # converts a correlation df into clean matrix
  corrr::fashion() %>%
  readr::write_excel_csv(here::here("Outputs","tucr_correlation-matrix.csv"))

tucr.cor.matrix.heatmap <- tucr.cor.matrix2 %>%
  corrr::correlate() %>%
  corrr::focus(1:482, mirror = TRUE) %>%
  # converts a correlation df into clean matrix
  corrr::fashion() %>%
  gather(key = "Partner", value = "Cor",-rowname) %>%
  mutate(corr = ifelse(Cor=="",1,Cor)) %>%
  select(TUCR = rowname,Partner,corr)

tucr.cor.matrix.heatmap$corr <- as.numeric(tucr.cor.matrix.heatmap$corr)
tucr.cor.matrix.heatmap$TUCR <- as.character(tucr.cor.matrix.heatmap$TUCR)
tucr.cor.matrix.heatmap$Partner <- as.character(tucr.cor.matrix.heatmap$Partner)


ggplot(data = tucr.cor.matrix.heatmap,mapping = aes(x = reorder(TUCR,corr),y = reorder(Partner,corr), fill = corr)) +
  geom_tile() +
 scale_fill_gradientn(colours = c("blue", "white", "red"), values = c(-0.25,0.625,1)) +
  theme(panel.grid = element_blank(),
        axis.text = element_blank(),
        axis.ticks = element_blank()) +
        ggsave(file=paste(outputdir,"/tucr_corr_heatmap.png",sep=""))

tucr.cor.matrix.heatmap %>%
  filter(abs(corr) >= 0.6 & abs(corr) < 1) %>%
  write_csv("topcorr.csv")
...
filename <- "tucroverexpression.csv"
x <- read.csv(filename)

p <- ggplot(x,aes(x = Target, y = FC, fill = Sample)) + 
  labs(title = "TUCR Overexpression in U251 cells") +
  ylab("Fold Change") +
...
ggsave(p,file=paste(filename,".png",sep=""), width = 7.5, height = 5, dpi = 300)
