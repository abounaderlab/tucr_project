...
tucrjoiner <- read.table("hg38.ultraconserved.bed",header=TRUE) 
...
spatialdata$mark <- factor(spatialdata$mark,      # Reordering group factor levels
                         levels = c("RNA Pol.II", "H3K4me3", "H3K27Ac"))
  
#p <- ggplot(tucr_lncRNAs, aes(x=row,y=value,fill=as.factor(fill)))                +
  #p <- ggplot(spatialdata, aes(x=reorder(annot,Order),y=as.numeric(as.character(invlog10_p)),fill=as.factor(Order1))) +
  p <- ggplot(spatialdata, aes(x=reorder(annot,Order),y=as.numeric(as.character(foldchange)),fill=as.factor(Order1)))   +
  geom_bar(
    stat="identity", 
    color = "black",width=0.7) +
  geom_errorbar(aes(ymax=as.numeric(as.character(errorbar)), ymin=as.numeric(as.character(foldchange))), width=.2,
                 position=position_dodge(.9)) +
  geom_text(aes(label = datalabels), nudge_y = 0.5, color = "black") +
  scale_fill_brewer(palette = "Spectral") +
  #scale_fill_manual(values = c("red3","green4","blue3","deeppink","yellow2","orange2","black","gray","violet","cyan")) +
  scale_y_continuous() +
  #geom_text(
  #  aes(label=round(value,2)), 
  #  vjust=1.6, 
  ...
  theme(
    ...
    axis.line = element_line(
      colour = "black")) +
    facet_wrap(~reorder(mark,spatialdata$Order2))

ggsave(
    #file=paste(outputdir,"/",disease,"_tucr_lncRNAs.png",sep=""),
    file=paste(outputdir,"/SpatialRelations/",disease,"_tucr_spatialrelations.png",sep=""),
    plot = print(p),
    width = 10, 
    height = 7.5, 
    dpi = 300) 
...
```{r Section2_DESeq2_TUCRs, eval = FALSE}
...
ggpca <- function (x, intgroup = "condition", ntop = 500, returnData = FALSE, labelpoints=FALSE)
{
  ...
  p <- ggplot(data = d, aes_string(x = "PC1", y = "PC2", color = "group", label="names")) +
    geom_point(size = 7) +
    scale_color_manual(labels = c("GOF-mut-p53 Tumor", "Normal Brain"), values = c("#f8766d", "#00bfc4")) +
...
write_csv(as.data.frame(pca$x),paste(outputdir,"/QualityControl/aboundaer-TUCR-qc-pca.csv",sep=""))
}

## PCA
rld <- varianceStabilizingTransformation(dds)

ggpca(rld, intgroup = c("dex"), labelpoints=FALSE) + xlim(-20, 35) + ylim(-15,15)
ggsave(paste(outputdir,"/QualityControl/aboundaer-TUCR-qc-pca.png",sep=""), w=15, h=10)
