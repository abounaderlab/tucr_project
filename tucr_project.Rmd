...
for(i in 1:length(chrom)){
  x <- bed %>%
    filter(V1 == chrom[i]) %>%
    write.table(paste("BEDFiles/",chrom[i],"_CHESSgenes.bed",sep=""),quote = F,col.names = F,row.names = F,sep = '\t')
}}

bedbreaker("BEDFiles/CHESSGenes.bed")
}
...
if(!is.na(n_countfile)){
  mergedcounts <- read_table(t_countfile)
