...
t_countfile <- paste(disease,"hg38.ultraConserved.counts.txt",sep="_")

#n_countfile <- paste(normal,"mergedcounts.txt",sep="_")
n_countfile <- paste(normal,"hg38.ultraConserved.counts.txt",sep="_")
...
echo -e chrom"\t"start"\t"end"\t"strand"\t"id"\t"tag"\t"annot RNASEQ-PROCESSED/tcga_analysis/GBM/reindex/*bam | sed -e "s/ /\t/g" > RNASEQ-PROCESSED/tcga_analysis/GBM/foldchange/$name2.counts.txt
...
if(!is.na(n_countfile)){
  ...
  normalcounts <- read.table(n_countfile,header = TRUE) %>%
    dplyr::select(-strand,-tag)
  
  mergedcounts <- mergedcounts %>% full_join(normalcounts,by=c("id")) %>%
    dplyr::select(-chrom.y,-start.y,-end.y,-annot.y)
...
if(!is.na(filterannot)){
  mergedcounts <- mergedcounts %>%
    filter(annot.x == filterannot)
}


mergedcounts2 <- 
  mergedcounts[,6:ncol(mergedcounts)]
...
ggsave(paste(outputdir,"/aboundaer-tucr-qc-pca.png",sep=""), w=10, h=10, scale=.9)
